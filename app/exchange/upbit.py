...
class UpbitClient:
    ...
    async def get_ticker(self) -> Ticker:
        async with httpx.AsyncClient(base_url=self.settings.upbit_base_url, timeout=5.0) as client:
            ticker_response = await client.get("/ticker", params={"markets": self.settings.market})
            ticker_response.raise_for_status()
            ticker_payload = ticker_response.json()
            if not ticker_payload:
                raise RuntimeError(f"No ticker returned for market {self.settings.market}")

            orderbook_response = await client.get("/orderbook", params={"markets": self.settings.market})
            orderbook_response.raise_for_status()
            orderbook_payload = orderbook_response.json()
            if not orderbook_payload:
                raise RuntimeError(f"No orderbook returned for market {self.settings.market}")
